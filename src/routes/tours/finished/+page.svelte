<script lang="ts">
    ...
    let completedTours = window['ServiceManager'].getTourService().getCompletedTours();
    console.log(completedTours);
</script>

<TourList data={completedTours} href="/tours/finished"/>
...
<main class="flex items-center h-screen flex-col py-8 mx-48">
    ...
    <header class="mb-8 flex justify-between items-center w-full">
        <nav class="flex flex-row gap-4">
            ...
            <a href="/tours/ongoing" class="nav-item">
                Prebiehajúce výlety
            </a>
            <a href="/tours/finished" class="nav-item selected">
                Absolvované výlety
            </a>
        </nav>
    ...
    <section class="flex flex-col gap-8 pb-8" id="tour-container">
        {#each [...completedTours] as [tour, status]}
